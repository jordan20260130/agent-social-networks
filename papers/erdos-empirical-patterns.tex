...
\title{Lessons from Erd\H{o}s: Empirical Patterns in Heterogeneous AI Collaboration}
...
\date{February 1, 2026 \\ \textit{Revised: February 6, 2026}}
...
The hypothesis that heterogeneous AI agent networks might exhibit collective intelligence has received significant theoretical attention \cite{manus2026roadmap, tomasev2025distributional, hammond2025multiagent}. However, empirical evidence for such emergence remains sparse. This paper addresses that gap by analyzing a unique dataset: the Erd\H{o}s Problems AI Contributions Wiki \cite{tao2026wiki}, which documents over 200 instances of AI tools contributing to open mathematical problems.

The Erd\H{o}s problems corpus---over 1,000 open problems in combinatorics and number theory posed by Paul Erd\H{o}s throughout his career---provides an ideal testbed for studying AI collaboration. Problems vary widely in difficulty, solutions can be formally verified (via Lean/Mathlib), and the mathematical community has established clear criteria for what constitutes progress.
...
\subsection{The Erd\H{o}s Problems Corpus}

Paul Erd\H{o}s (1913--1996) posed thousands of problems throughout his career, many of which remain open. The Erd\H{o}s Problems website (\url{https://www.erdosproblems.com/}) catalogs over 1,000 such problems with standardized metadata: problem statements, known partial results, relevant literature, and community discussion forums \cite{erdosproblems2026}.
...
Our contribution is to ground these proposals in empirical data from the Erd\H{o}s wiki.

\section{Methodology}

We analyzed the Erd\H{o}s Problems AI Contributions Wiki as of February 1, 2026, including:
\begin{itemize}
...
For each entry, we recorded: problem ID, AI tools used, date, outcome (full/partial/failure), whether multiple tools were used, whether humans were involved, and whether Lean formalization was produced.
...
DeepResearch $\times$ 2 & Parallel Literature Search & \#281, \#367, \#481, \#543 \\
...
The pattern is consistent: \textbf{one system for exploration/reasoning, another for verification/formalization}. Single-tool approaches in Section 1(a) show higher failure rates than multi-tool approaches in Sections 1(c) and 1(d).
...
\subsection{Finding 2: Independent Validation via AB-MCTS}

Subsequent to our initial analysis, Sakana AI published empirical validation of the Social-Scale MoE hypothesis through their AB-MCTS (Adaptive Branching Monte Carlo Tree Search) system \cite{sakana2025abmcts}. AB-MCTS combines multiple LLMs (o4-mini, Gemini-2.5-Pro, DeepSeek-R1-0528) using Thompson Sampling to select models at each search node.

The key result: AB-MCTS significantly outperforms any individual model on ARC-AGI-2, a challenging reasoning benchmark. This confirms our Finding 1 at scale and demonstrates that:
\begin{enumerate}
    \item Model selection as \textit{routing} (not ensembling) improves performance
    \item Bayesian uncertainty prevents premature consensus
    \item Adaptive branching naturally balances exploration vs.~exploitation
\end{enumerate}

This independent validation strengthens the empirical basis for heterogeneous agent architectures.

\subsection{Finding 3: Verification is the Bottleneck}

Many failure results stem from plausible-looking arguments that fail on inspection. Problems \#51, \#233, \#616, \#647, and \#888 all involved ``incorrect proofs found.''
...
\subsection{Finding 4: Literature Synthesis is a Capability Gap}
...
Problem \#281 is instructive: ChatGPT DeepResearch, Gemini DeepResearch, and Claude all failed to find that combining Davenport-Erd\H{o}s (1936) with Rogers (1966) would solve the problem. \textbf{Cross-paper synthesis}---detecting when the output of one research stream satisfies the requirements of another---represents a significant capability gap.

\subsection{Finding 5: Human-AI Complementarity}

Section 1(d) documents the strongest results---problems solved by humans working with AI:
\begin{itemize}
    \item \textbf{\#347}: 3-month collaboration (4 humans + GPT Codex + Aristotle) $\rightarrow$ Lean
    \item \textbf{\#848}: 6-week collaboration (Sawhney, Sellke + GPT-5) $\rightarrow$ Full solution
    \item \textbf{\#1026}: Multi-human, multi-AI team $\rightarrow$ Lean
\end{itemize}
...
\subsection{Finding 6: The Caveats Are Design Principles}
...
Based on these findings, we propose refinements to Manus AI's three experiments \cite{manus2026roadmap}, incorporating critical insights from the multi-agent systems literature.

\subsection{Critical Warning: Debate Can Harm Accuracy}

A comprehensive literature review \cite{wynn2025talk} reveals a crucial finding: multi-agent debate does not inherently improve accuracy. Models frequently shift from correct to incorrect answers in response to peer reasoning, favoring agreement over challenging flawed reasoning. This ``agreement bias'' means:
\begin{itemize}
    \item Simple majority voting is dangerous
    \item ``Debate until agreement'' is an anti-pattern
    \item Verification must be independent, not consensus-seeking
\end{itemize}

This warning fundamentally shapes our refined proposals below.

\subsection{Experiment 1: Market for Scientific Lemons (Refined)}

\textbf{Refinement}: Use \textbf{Lean-verifiable mathematical claims} as the testbed with \textit{independent} verification.
...
\begin{enumerate}
    ...
    \item Verification agents verify \textit{independently} (no access to other verifiers' conclusions)
    \item Successful formalization rewards both claimant and verifier
    \item Failed formalization penalizes the claimant
\end{enumerate}

This models the observed ChatGPT $\rightarrow$ Aristotle workflow while avoiding agreement bias.

\subsection{Experiment 2: Emergent Discovery Sandbox (Refined)}

\textbf{Refinement}: Replicate the observed tool ecosystem with \textit{learned consensus}:
\begin{itemize}
...
\textbf{Measurable hypothesis}: Heterogeneous networks with learned consensus (weighted by past reliability) achieve higher success rates than homogeneous populations of equal compute.
...
\begin{enumerate}
    ...
    \item Fixed rounds of exchange; no ``debate until agreement''
    \item Human review if no consensus after max rounds
    \item Successful synthesis $\rightarrow$ shared credit
\end{enumerate}

This directly targets the observed synthesis gap while avoiding the agreement bias pitfall.
...
\begin{tabular}{lll}
\toprule
\textbf{Metric} & \textbf{Definition} & \textbf{Erd\H{o}s Analog} \\
\midrule
Solution Rate & \% attempts $\rightarrow$ full or partial & Wiki classification \\
...
\bottomrule
\end{tabular}
\caption{Proposed metrics grounded in Erd\H{o}s data}
\label{tab:metrics}
\end{table}
...
The Erd\H{o}s data and subsequent AB-MCTS validation suggest that agent social networks should be designed for:
\begin{enumerate}
    ...
    \item \textbf{Independent verification}: Avoid agreement-seeking corruption
    \item \textbf{Learned consensus}: Weight by reliability, not simple voting
...
The Erd\H{o}s Problems wiki provides rare longitudinal data on heterogeneous AI collaboration. Our analysis reveals consistent patterns: specialized tool combinations outperform, verification is the bottleneck, and synthesis represents a capability gap. Independent validation through Sakana AI's AB-MCTS confirms that heterogeneous multi-LLM ensembles significantly outperform individual models on reasoning benchmarks.

These findings ground the theoretical proposals of the \texttt{agent-social-networks} project in empirical observation. The three experiments proposed by Manus AI are sound; we refine them with concrete protocols derived from observed workflows, critical warnings from the literature, and strategic directions for agentic innovation.
...
\textit{AI contributions to Erd\H{o}s problems}.
GitHub Wiki.
...
\bibitem{sakana2025abmcts}
Sakana AI (2025).
\textit{AB-MCTS: Adaptive Branching Monte Carlo Tree Search for LLM Reasoning}.
arXiv:2503.04412.
\url{https://arxiv.org/abs/2503.04412}

\bibitem{wynn2025talk}
Wynn, T., Satija, H., \& Hadfield, G.~(2025).
\textit{Talk Isn't Always Cheap: Eliciting Truthful Communication in Multi-Agent Debate}.
arXiv:2502.07601.
